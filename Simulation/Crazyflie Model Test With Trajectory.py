import gymnasium as gym
import argparse
from stable_baselines3 import DQN
from stable_baselines3 import PPO
from stable_baselines3 import DDPG
from stable_baselines3.common.policies import ActorCriticPolicy
import keyboard
from gym_pybullet_drones.utils.enums import ObservationType, ActionType
from stable_baselines3.common.env_util import make_vec_env
from gym_pybullet_drones.envs.HoverAviary import HoverAviary
from stable_baselines3.common.callbacks import EvalCallback, StopTrainingOnRewardThreshold
import os
from datetime import datetime
from gym_pybullet_drones.utils.enums import DroneModel, Physics, ActionType, ObservationType, ImageType
import numpy as np
import sys
from collections import deque

'''
This program tests a trained MLP model or policy on the HoverAviary environment with a GUI.
The quadrotor is controlled to follow a predefined trajectory defined in the Trajectory() method.
This can be useful to visualize how well the model performs in simulation.
Nicholas Navarrete
'''

HOVER_RPM=55996
MAX_RPM=65534

history = [deque(maxlen=5) for _ in range(4)]

def colorize(pct):
    """Return colored percentage string with green→yellow→red transition in 80–90%."""
    reset = "\033[0m"
    
    if pct < 80:
        color = "\033[92m"   # Green
    elif pct >= 90:
        color = "\033[91m"   # Red
    else:
        # Map 80–90 → [0,1]
        t = (pct - 80) / 10.0
        if t < 0.5:
            color = "\033[93m"  # Yellow (midway)
        else:
            color = "\033[91m"  # Red (approaching 90%)
    
    return f"{color}{pct:3d}%{reset}"

def display_quadrotor(action):
    """
    Display quadrotor motors in X configuration with smooth color scaling in 80–90%.
    Overwrites the same spot in the console each update.
    action: numpy array or list with 4 motor values (0–65534).
    """
    # Normalize 0–65534 to percentage
    global history
    
    percentages = [(a / 65534) * 100 for a in action[0,0]]
    
    # Update history
    for i, p in enumerate(percentages):
        history[i].append(p)
    
    # Compute rolling average
    smoothed = [sum(h)/len(h) for h in history]
    
    m1, m2, m3, m4 = [colorize(int(p)) for p in smoothed]

    lines = [
        f" m4({m4})        m1({m1})",
        f"      \\      /",
        f"         X",
        f"      /      \\",
        f" m3({m3})        m2({m2})"
    ]

    print("\n".join(lines))
    sys.stdout.write("\033[5A")
    sys.stdout.flush()

def _preprocess_action(action):
        ''' This Function Preprocess the action from the PPO model to the Crazyflie setpoint format.
            PPO outputs the action in the format: RPMm1 = self.hover_rpm *(1+0.05*thrust)
            Where the crazyflie setpoint wants rpms
        '''
        
        #print(self.RunTime ,int(action[0,0,0]), int(-action[0,0,1]), int(action[0,0,2]), int(action[0,0,3]), "\t", end="\r")
        # The action is in the format: [[[m1, m2, m3, m4]]]

        """Note: the motors layout changed so that m1 and m3 flip positions & m4 and m2 flip positions"""
        m1 = HOVER_RPM + HOVER_RPM* 0.05 * action[0,0,0]
        m2 = HOVER_RPM + HOVER_RPM* 0.05 * action[0,0,1]
        m3 = HOVER_RPM + HOVER_RPM* 0.05 * action[0,0,2]
        m4 = HOVER_RPM + HOVER_RPM* 0.05 * action[0,0,3]



        # Make sure the motors are not above the maximum RPM
        m1 = min(m1, MAX_RPM)
        m2 = min(m2, MAX_RPM)
        m3 = min(m3, MAX_RPM)
        m4 = min(m4, MAX_RPM)
        # Return the action in the format: [m1, m2, m3, m4]
        return np.array([[[m1, m2, m3, m4]]])

def detect_rotation(x, y, prev_angle):
    import numpy as np
    angle = np.arctan2(y, x)
    rotation = 0
    
    if prev_angle is not None:
        # Detect wrap-around from +pi to -pi
        if prev_angle > 2.5 and angle < -2.5:
            rotation = 1

    return angle, rotation

def Trajectory(Time):
    """
    This function defines a trajectory as a function of time and returns the desired positions
    """
    #Start at 0, then over 5 seconds go out 1 radius
    if Time < 5:
        radius = 0.1 * Time  # meters
        x=radius
        y=0
        z=1
    # Circle trajectory
    else:
        Time_offset = Time - 5
        radius = 0.5  # meters
        omega = 0.8   # radians per second
        x = radius * np.cos(omega * Time_offset)
        y = radius * np.sin(omega * Time_offset)
        z = 1       # constant height

    return np.array([x, y, z])

Sim_Freq=12000
control_freq = 120 #in Hz

DEFAULT_OBS = ObservationType.REAL2 # 'kin' or 'rgb'
DEFAULT_ACT = ActionType('rpm') # 'rpm' or 'pid' or 'vel' or 'one_d_rpm' or 'one_d_pid'


actual_x = []
actual_y = []

target_x = []
target_y = []

num_rotations = 0
prev_angle = None
completed_one_rotation = False

initial_pos = np.array([[0,0,1]])
initial_pos.reshape(1,3)
# env = make_vec_env(HoverAviary,env_kwargs=dict(obs=ObservationType.REAL,
#                                                 act=ActionType.RPM, 
#                                                 ctrl_freq=control_freq, 
#                                                 pyb_freq=Sim_Freq, 
#                                                 initial_xyzs = initial_pos, 
#                                                 drone_model = DroneModel.CF2X, 
#                                                 gui = True, 
#                                                 randomize_init_rpy=True,
#                                                 GoalPos=False),n_envs=1)
env = HoverAviary(obs=ObservationType.REAL3,
                        act=ActionType.RPM, 
                        ctrl_freq=control_freq, 
                        pyb_freq=Sim_Freq, 
                        initial_xyzs = initial_pos, 
                        drone_model = DroneModel.CF2X, 
                        gui = True, 
                        randomize_init_rpy=True,
                        GoalPos=False)
obs, info = env.reset()


name = "SimToReal63_policy.zip"
if "policy" not in name:
    model = PPO.load(name,print_system_info=True)
else:
    model = ActorCriticPolicy.load(name)


Time = 0
print(env.action_space)
while not keyboard.is_pressed('esc'):
    Goal = Trajectory(Time)
    env.UpdateTarget(Goal)
    action, _states = model.predict(obs, deterministic=True)
    # --- Target trajectory ---
    target_x.append(Goal[0])
    target_y.append(Goal[1])

    # --- Actual drone position (obs contains Goal - actual) ---
    drone_x = Goal[0] - obs[0,0]
    drone_y = Goal[1] - obs[0,1]

    actual_x.append(drone_x)
    actual_y.append(drone_y)

    # --- Detect rotation ---
    angle, rot = detect_rotation(Goal[0], Goal[1], prev_angle)
    prev_angle = angle
    num_rotations += rot

    # Stop after 1 full rotation
    if num_rotations >= 1:
        completed_one_rotation = True
        break
    #action2 = _preprocess_action(action)
    #display_quadrotor(action2)
    obs, reward, terminated, truncated, info = env.step(action)
    Time += 1/control_freq
    #print(str(action[0,0,0]) + '\r')
    if keyboard.is_pressed('r'):
        obs, info = env.reset()
        Time=0
        

import matplotlib.pyplot as plt

plt.figure(figsize=(6,6))
plt.plot(target_x, target_y, label="Intended Trajectory", linewidth=3)
plt.plot(actual_x, actual_y, label="Drone Path", linestyle='--')

plt.xlabel("X position (m)")
plt.ylabel("Y position (m)")
plt.title("Top-Down XY Flight Path – Half Rotation - "+name)
plt.legend()
plt.axis("equal")  # keep circle shape
plt.grid(True)
plt.show()