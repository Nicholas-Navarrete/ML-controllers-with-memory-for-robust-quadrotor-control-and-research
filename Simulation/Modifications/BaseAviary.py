...
Updated _storekineamatic.... to include pendulum angle and angular velocity
Added all instances of self.penAng and self.penAngVel

Gave the pendulum angle a varance of 20
...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        self.Inefficient_Motors = Inefficient_Motors
        self.randomize_init_rpy = randomize_init_rpy
        self.kf_variance = kf_Variance
        #### Constants #############################################
        self.G = 9.8
        ...
        self.roll_rate_buff = 0
        self.pitch_rate_buff = 0
        self.yaw_rate_buff =0 
        ...
        elif self.DRONE_MODEL == DroneModel.CF2XINVPEN: #  ADDED THIS CASE
            self.MAX_XY_TORQUE = (2*self.L*self.KF*self.MAX_RPM**2)/np.sqrt(2)
    ...
    def _housekeeping(self):
        ...
        #CHANGE, ADDING A STARTING POS OF 2 meters in the air
        self.pos[:, 2] = 2 ##This method is not working
        ...
        self.rpy_offset = np.array([random.gauss(0,0.01) for _ in range(3)])
        self.rpy_offset = np.array([0, 0, 0])
        # Added self.penAng penAngVel
        self.penAng = np.zeros((self.NUM_DRONES, 2), dtype=np.float64)
        self.penAngVel = np.zeros((self.NUM_DRONES, 2), dtype=np.float64)
        ...
        # Update the randomizations
        if self.randomize_init_rpy:
            self.INIT_RPYS = np.array([
                [random.gauss(0, 0.1), random.gauss(0, 0.1), random.gauss(0, 0.1)]
                for _ in range(self.NUM_DRONES)
            ])

        if self.Inefficient_Motors == True:
            self.KF_Offset = np.array([random.gauss(0,self.kf_variance) for _ in range(4)])
        ...
        if self.DRONE_MODEL == DroneModel.CF2XINVPEN:
            for x in range(self.NUM_DRONES):
                for i in range(p.getNumJoints(self.DRONE_IDS[x])):
                    joint_name = p.getJointInfo(self.DRONE_IDS[x], i)[1].decode("utf-8")
                    if joint_name == "pendulum_x_joint":
                        p.resetJointState(self.DRONE_IDS[x], i, math.radians(random.gauss(0,10)))
                        p.setJointMotorControl2(self.DRONE_IDS[x], i,controlMode=p.VELOCITY_CONTROL,force=0)
                    elif joint_name == "pendulum_y_joint":
                        p.resetJointState(self.DRONE_IDS[x], i, math.radians(random.gauss(0,10)))
                        p.setJointMotorControl2(self.DRONE_IDS[x], i,controlMode=p.VELOCITY_CONTROL,force=0)
                        #print('y angle    ' +str(p.getJointState(self.DRONE_IDS[x], i, physicsClientId=self.CLIENT))) 
    ...
    def _updateAndStoreKinematicInformation(self):
        ...
        for i in range (self.NUM_DRONES):
            ...
            if self.DRONE_MODEL == DroneModel.CF2XINVPEN:   ### ADDED THIS FUNCTION
                    
                # --- Pendulum joint angles and velocities ---
                # Assumes you know the joint indices or names
                pendulum_x_joint_idx = None
                pendulum_y_joint_idx = None

                # Search all joints for the right names
                for j in range(p.getNumJoints(self.DRONE_IDS[i], physicsClientId=self.CLIENT)):
                    joint_name = p.getJointInfo(self.DRONE_IDS[i], j, physicsClientId=self.CLIENT)[1].decode("utf-8")
                    if joint_name == "pendulum_x_joint":
                        pendulum_x_joint_idx = j
                    elif joint_name == "pendulum_y_joint":
                        pendulum_y_joint_idx = j
                
                
                x_state = p.getJointState(self.DRONE_IDS[i], pendulum_x_joint_idx, physicsClientId=self.CLIENT)
                y_state = p.getJointState(self.DRONE_IDS[i], pendulum_y_joint_idx, physicsClientId=self.CLIENT)
                
                # Store pendulum angles and angular velocities
                self.penAng[i] = [x_state[0], y_state[0]]        # Joint positions (radians)
                self.penAngVel[i] = [x_state[1], y_state[1]]     # Joint velocities (rad/s)
    ...
    def _getDroneStateVector(self,
        ...
        ndarray 
            ...
            of a 24 shapped array if we have the inverted pendulum

        """
        if self.DRONE_MODEL == DroneModel.CF2XINVPEN:
            state = np.hstack([self.pos[nth_drone, :], self.quat[nth_drone, :], self.rpy[nth_drone, :],
                            self.vel[nth_drone, :], self.ang_v[nth_drone, :], self.penAng[nth_drone,:],self.penAngVel[nth_drone, :],self.last_clipped_action[nth_drone, :]])
    ...
    def _physics(self,
        ...
        """
        #if not self.Inefficient_Motors:
            #forces = np.array(rpm**2)*self.KF
        #else:
            #forces = np.array([rpm[i]**2*self.KF[i] for i in range(4)])
        if  self.Inefficient_Motors == False:
            forces = np.array(rpm**2)*(self.KF)
        else:
            if (self.KF+self.KF_Offset).min() < 0:
                self.KF_Offset += abs((self.KF+self.KF_Offset).min())
            forces = np.array(rpm**2)*(self.KF+self.KF_Offset)
        ...
        prop_links = [0, 1, 2, 3]   # prop0, prop1, prop2, prop3
    ...
    def _dynamics(self,
        ...
        if self.DRONE_MODEL==DroneModel.CF2X or self.DRONE_MODEL==DroneModel.RACE or self.DRONE_MODEL==DroneModel.CF2XINVPEN: # Added invpen cases
    ...
    def _Attitude_Rate_PID_Simulation(self,states,action):
        # Where the action is [rollRate, pitchRate, yawRate, Thrust]
        # No filtering or simulation of acceleration rate is performed, currently
        # It is just the difference with the last time step

        # Future work is to add a seperate PID attitude rate sim frequency which is approximately 500 Hz
        ...
        roll = np.rad2deg(states[7])
        pitch = -np.rad2deg(states[8])
        yaw = np.rad2deg(states[9])
        
        roll_rate = np.rad2deg(states[13])
        pitch_rate = -np.rad2deg(states[14])
        yaw_rate = np.rad2deg(states[15])


        roll_output = roll_kp*(roll_rate-action[0])
        pitch_output = pitch_kp*(pitch_rate+action[1]) # The action is inverted to account for the z axis inversion on the quad
        yaw_output = yaw_kp*(yaw_rate-action[2])
        thrust = self.HOVER_RPM * (1+0.05*action[3])
        #thrust = self.MAX_RPM * action[3]
        
        # From crazyflie-firmware/src/modules/src/power_distribution_quadrotor.c

        m1 = thrust - roll_output + pitch_output + yaw_output
        m2 = thrust - roll_output - pitch_output - yaw_output
        m3 = thrust + roll_output - pitch_output + yaw_output
        m4 = thrust + roll_output + pitch_output - yaw_output
        
        maximumThrust = max(m1,m2,m3,m4)
        # Now we need to cap the motor speed
        if maximumThrust > self.MAX_RPM:
            reduction = maximumThrust - self.MAX_RPM
            m1 = m1 - reduction
            m2 = m2 - reduction
            m3 = m3 - reduction
            m4 = m4 - reduction
