...
SimToReal_37 changes the reward to omit the rotational velocity reward and increases the obs size from the last 10 obs to the last 100 obs This learns too slow
...
SimToReal_55 increases the variance of KF offset to 1e-11 from 1.63e-17
SimToReal_56 tests to ensure the system still works well reduces learning time from 1e7 to 5e6 This does nto work
SimToReal_57 tries to fix above This flies
SimToReal_58 reduces the learning time to 5e6 This flies well Inefficient_motors = False
SimToReal_59 sets Inefficient_motors = True variance is 1e-11 This flies badly.
SimToReal_60 uses the REAL observation type insterad of REAL3
SimToReal_61 turns off inefficient motors
SimToReal_62 uses RPM2 action type This does not fly
SimToReal_63 uses obs type REAL3 and action type RPM with inefficient motors on
'''
...
target_reward = 10000
filename = os.path.join('SimToReal', 'save-'+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")) ##was InvPen
...
model = PPO('MlpPolicy',
             ...
             batch_size= 512,
             verbose=1,
             learning_rate=lambda frac: 5e-5 + (5e-5 * np.real(frac**0.5)),
             n_epochs=3,
             n_steps=4096,
             ent_coef = 0.01,
             clip_range=0.2,
...
model.learn(total_timesteps= 5e6,
            ...
            log_interval=10, progress_bar=True,tb_log_name="SimToReal")

model.save("PPO_SimToReal")#was PPO_Latest_InvPen

env = HoverAviary(gui=True, ctrl_freq=control_freq, pyb_freq=Sim_Freq,obs=ObservationType.REAL, act=ActionType.RPM2,drone_model = DroneModel.CF2X, initial_xyzs=training_initial_pos, Inefficient_Motors=True )
obs,info = env.reset()
