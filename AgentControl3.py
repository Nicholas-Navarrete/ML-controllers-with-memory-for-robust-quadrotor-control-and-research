...
Agent Control 3 Allows for the control of LSTM modules and like Agent Control 2 it Implements the VICON dssdk to get the position of the drone and uses a trained PPO model
with xyz rpy Vxyz Wxyz Avg_Action Delta_Action as the observation space to control the drone in position hold.
...
class DroneController:
    ...
    def __init__(self, link_uri, model_name=None, LSTM = False, ActionType = 'RPM'):
        ...
        # Action Buffers
        self.LastAction = np.zeros((4))
        self.LastActions = np.zeros((5,4))
       
        if model_name:
            if LSTM == False:
                self.model=ActorCriticPolicy.load(model_name, device = 'cuda')
            if LSTM == True:
                self.model=RecurrentActorCriticPolicy.load(model_name, device = 'cuda')
    ...
    def _get_observations(self):
        ...
        pos_raw = self.client.GetSegmentGlobalTranslation(drone_object_name, drone_object_name)[0]
        pos = np.array(pos_raw) / 1000.0  # Convert from mm to m
        ...
        if (abs(rpy[0]) > np.pi/2) or (abs(rpy[1]) > np.pi/2): # If the drone is flipped, shut off the motors and end the program
           self.control = False
           print("Drone flipped, shutting down"+str(np.rad2deg(rpy)))
        ...
        avg_action = np.mean(self.LastActions[:5], axis=0)
        delta_action = self.LastActions[0] - self.LastActions[1]

        return np.array([[[pos[0],  pos[1], pos[2], 
                            rpy[0], rpy[1], rpy[2], 
                            Vx, Vy, Vz, 
                            Wx, Wy, Wz,
                            avg_action[0],  avg_action[1],  avg_action[2],  avg_action[3],
                            delta_action[0],delta_action[1],delta_action[2],delta_action[3]]]])
    ...
    def _preprocess_action(self, action):
        ...
        # Shift all old actions one step towards the end
        self.LastActions[1:] = self.LastActions[:-1]
        # Store newest action at index 0
        self.LastActions[0] = action

        Power_Variance = 0.1 #A factior +/- the hover rpm that the action can command
        
        m1 = self.HOVER_RPM + self.HOVER_RPM* Power_Variance * action[0,0,0]
        m2 = self.HOVER_RPM + self.HOVER_RPM* Power_Variance * action[0,0,1]
        m3 = self.HOVER_RPM + self.HOVER_RPM* Power_Variance * action[0,0,2]
        m4 = self.HOVER_RPM + self.HOVER_RPM* Power_Variance * action[0,0,3]
        ...
        while self.control:
            ...
            if  1==1:#self.RunTime - self.timer > 1/self.CONTROLFREQUENCY:
                ...
                obs = np.array([[[Target[0]- obs[0,0,0], Target[1]-obs[0,0,1], Target[2]-obs[0,0,2],
                                  ...
                                  obs[0,0,12],  obs[0,0,13],    obs[0,0,14],    obs[0,0,15],
                                  obs[0,0,16],  obs[0,0,17],    obs[0,0,18],    obs[0,0,19]]]])
                
                

                if self.LSTM == True:
                    action, self.lstm_states = self.model.predict(obs, state = self.lstm_states, deterministic=True)
                else:
                ...
                #display_action = self._preprocess_action(action)
                #display_quadrotor(display_action)

                #print("Action: ", int(0xFFFF*action[0,0]), int(0xFFFF*action[0,1]), int(0xFFFF*action[0,2]), int(0xFFFF*action[0,3]), "\t", end="\r")
                #if self.ActionType == 'RPM2':
                    #self._cf.commander.send_setpoint(int(0xFFFF*action[0,0]), int(0xFFFF*action[0,1]), int(0xFFFF*action[0,2]), int(0xFFFF*action[0,3]))
                #else:
...
if __name__ == '__main__':
    ### I might have to unzip the policies
    print('starting')
    # Initialize the low-level drivers
    time.sleep(5)
    cflib.crtp.init_drivers()
    print('drivers initialized')
    controller = DroneController(uri, model_name = "SimToReal63_policy.zip", LSTM = False, ActionType='RPM2')
